//! Card action response types and handler.
...
type CardCallback = Box<dyn Fn(Event) -> CardResponse + Send + Sync + 'static>;

/// Built-in handler for `MessageType::Card`.
///
/// Note: `card.action.trigger` events arrive with frame header `type=event`
/// (not `type=card`).  For WebSocket, use `EventHandler` and call
/// `Event::card_action()`.  This handler is for the HTTP callback path.
...
impl CardActionHandler {
    ...
    where
        F: Fn(Event) -> CardResponse + Send + Sync + 'static,
    {
...
impl MessageHandler for CardActionHandler {
    ...
    async fn handle(&self, msg: IncomingMessage) {
        ...
        let response = (self.callback)(event);
        let response_json =
